...
int RGWRados::init_complete()
{
  ...
  /* not point of running sync thread if there is a single zonegroup and a single zone or
     we don't have a master zone configured or there is no rest_master_conn */
  if ((current_period.is_single_zonegroup(cct, this) && get_zonegroup().zones.size() < 2) ||
      get_zonegroup().master_zone.empty() || !rest_master_conn) {
